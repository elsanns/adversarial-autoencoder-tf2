...
@tf.function
def train_step(x_batch, y_labels, gan, optimizers_dict, label_sample,
               real_distribution, n_classes):
    ...
    trainable_variables = gan.encoder.trainable_variables \
                          + gan.decoder.trainable_variables
    gan_gradients = gan_tape.gradient(gan_loss, trainable_variables)
    optimizers_dict['gan'].apply_gradients(
        zip(gan_gradients, trainable_variables))
    ...
    with tf.GradientTape() as discriminator_tape:
        ...
        fake_distribution_label = tf.concat([fake_distribution, y_labels],
                                            axis=1)
    ...
    encoder_gradients = encoder_tape.gradient(encoder_loss,
                                              gan.encoder.trainable_variables)
    optimizers_dict['encoder'].apply_gradients(
        zip(encoder_gradients, gan.encoder.trainable_variables))

    return gan_loss, discriminator_loss, encoder_loss


def train_all_steps(gan, optimizers_dict, train_ds, n_epochs, prior_type,
    ...
    for epoch in range(n_epochs):
        for batch_no, (x_batch, y_labels) in enumerate(train_ds):
            label_sample = np.random.randint(0, n_classes,
                                             size=[x_batch.shape[0]])
            real_distribution = plot_factory.prior_factory.get_prior(
                prior_type)(x_batch.shape[0], label_sample,
                            n_classes)

            gan_loss, discriminator_loss, encoder_loss = train_step(x_batch,
                                                                    y_labels,
                                                                    gan,
                                                                    optimizers_dict,
                                                                    label_sample,
            ...
            with summary_writer.as_default():
                tf.summary.scalar('gan_loss', gan_loss_vec.result(),
                                  step=optimizers_dict['gan'].iterations)
                tf.summary.scalar('encoder_loss', encoder_loss_vec.result(),
                                  step=optimizers_dict['encoder'].iterations)
                tf.summary.scalar('discriminator_loss',
                                  discriminator_loss_vec.result(),
                                  step=optimizers_dict[
                                      'discriminator'].iterations)

        print(
            "Epoch: {} total_loss: {} gan_loss: {}, discriminator_loss: {} encoder_loss: {}".
            format(epoch,
        ...
        visualize_results(gan, data_loader, plot_factory, n_classes,
                          prior_type, epoch)


def visualize_results(gan, data_loader, plot_factory, n_classes, prior_type,
                      epoch=None):
    ...
    x_test_img = x_test.reshape(n_tot_imgs, data_loader.img_size_x,
                                data_loader.img_size_y)
    plot_factory.plot_image_array_reconstr(x_test_img, name="x_input.png")

    # Test data for distribution plot
    x_dist, id_dist = data_loader.get_test_sample(dist_sample_count,
                                                  dist_sample_count)
    ...
    # Plot reconstruction
    y_reconstruction = gan.decoder(gan.encoder(x_test, training=False),
                                   training=False)
    y_reconstruction_img = tf.reshape(y_reconstruction,
                                      (n_tot_imgs, data_loader.img_size_x,
                                       data_loader.img_size_y)).numpy()
    plot_factory.plot_image_array_reconstr(y_reconstruction_img,
                                           name="/x_reconstruction_{}.png".format(
                                               epoch))
    ...
    if prior_type == 'gaussian_mixture':
        ...
        y_for_z_samples_img = tf.reshape(y_for_z_samples,
                                         (n_tot_imgs_sampled,
                                          ...
                                          data_loader.img_size_y)).numpy()
        plot_factory.plot_image_array_sampled(y_for_z_samples_img,
                                              name="z_sampled_reconstruction_{}.png".format(
                                                  epoch))

    # z distribution by label
    z_dist = gan.encoder(x_dist, training=False)
    plot_factory.plot_distribution(z_dist, id_dist,
                                   name="/z_distribution_{}.png".format(epoch))
