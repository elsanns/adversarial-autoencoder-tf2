...
@tf.function
def train_step(
    x_batch, y_labels, gan, optimizers_dict, label_sample, real_distribution, n_classes
):
    ...
    trainable_variables = (
        gan.encoder.trainable_variables + gan.decoder.trainable_variables
    )
    ...
    discriminator_gradients = discriminator_tape.gradient(
        discriminator_loss, gan.discriminator.trainable_variables
    )
    ...
    encoder_gradients = encoder_tape.gradient(
        encoder_loss, gan.encoder.trainable_variables
    )
...
def train_all_steps(
    ...
    for epoch in range(n_epochs):
        for batch_no, (x_batch, y_labels) in enumerate(train_ds):
            ...
            gan_loss, discriminator_loss, encoder_loss = train_step(
                x_batch,
                y_labels,
                gan,
                optimizers_dict,
...
def visualize_results(
    gan, data_loader, plot_factory, n_classes, prior_type, epoch=None
):
