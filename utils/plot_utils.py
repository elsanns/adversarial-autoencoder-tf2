...
class PlotFactory:
    def __init__(
        ...
        self.results_dir_current = os.path.join(
            self.results_dir,
            (prior_type + "_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")),
        )
    ...
    def merge_images(images):
        ...
        img_arr = reduce(
            (lambda x, y: np.concatenate((x, y), axis=0)), tuple(x for x in rows)
        )
    ...
    def plot_image_array_sampled(self, images, name="results.png"):
        self.plot_image_array(
            images, self.x_sampling_reconstr, self.y_sampling_reconstr, name
        )
    ...
    def plot_sampling_reconstr(self):
        ...
        z_sample = np.rollaxis(
            np.mgrid[
                x_range : -x_range : self.x_sampling_reconstr * 1j,
                x_range : -x_range : self.y_sampling_reconstr * 1j,
            ],
            0,
            3,
        )
